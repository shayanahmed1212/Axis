// Horizontal-scrolling week strip with a task list filtered by day + tab.
//
// Key scrolling logic:
//   - [_generateScrollDays] builds a 61-day window centred on the selected
//     date so there's always buffer to scroll left/right without hitting
//     an edge.
//   - [_centerDate] finds the index of the target date, re-generates the
//     window if the date fell out of range, then calls [_scrollToIndex]
//     to animate the strip so the target day lands in the horizontal centre
//     of the screen (`(index * itemWidth) - (screenWidth / 2) + (itemWidth / 2)`).
//
// Event dots: a 5px circle is rendered below each date label when
// [dayHasTasks] is true. The check scans every task's `dueDate` —
// for large task lists this should be cached/aggregated to avoid O(n)
// per day cell per frame (see `_buildWeekStrip` line where `dayHasTasks`
// is computed inside `itemBuilder`).
